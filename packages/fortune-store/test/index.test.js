...
import { FortuneStore, schemaToFortune } from '../src/index';
...
async function makeStore() {
  const store = new fortune(schemaToFortune(schema));

  await store.create('bears', { id: '1', ...attrs.bears[1] });
  await store.create('bears', { id: '2', ...attrs.bears[2] });
  await store.create('bears', { id: '3', ...attrs.bears[3] });
  await store.create('bears', { id: '5', ...attrs.bears[5] });

  await store.create('homes', { id: '1', ...attrs.homes[1], bears: ['1', '2', '3'] });
  await store.create('homes', { id: '2', ...attrs.homes[2], bears: [] });

  await store.create('powers', {
    id: 'careBearStare',
    ...attrs.powers.careBearStare,
    bears: ['1', '2', '3'],
  });
  await store.create('powers', { id: 'makeWish', ...attrs.powers.makeWish, bears: [] });

  return FortuneStore(schema, store);
}
...
test.skip('fetches multiple relationships of various types', async t => {
  const result = await t.context.store.get({
    type: 'bears',
    id: '1',
    relationships: {
...
test.skip('handles relationships between the same type', async t => {
  const result = await t.context.store.get({
    type: 'bears',
    relationships: {
      best_friend: {},
...
test.skip('updates existing objects', async t => {
...
test.skip('updates with one-to-many relationship', async t => {
  await t.context.store.update({
    type: 'bears',
    id: '1',
...
test.skip('updates with a many-to-one relationship', async t => {
  await t.context.store.update({
    type: 'homes',
    id: '1',
...
test.skip('updates with many-to-many relationship', async t => {
  await t.context.store.update({
    type: 'powers',
    id: 'makeWish',
